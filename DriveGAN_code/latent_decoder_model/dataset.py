...
class ImagesDataset(Dataset):
    def __init__(self, path, dataset, size, args=None, train=True):
        ...
        for idx, element in enumerate(os.listdir(path)):
            ...
            if os.path.isdir(elementpath):
                ...
                image_paths = image_paths + [path for path in glob(elementpath + "/*.png")]
        ...
        print('\n\n\n\n\n\n' + self.dataset + ', Number of data: ' + str(len(self.image_paths)) + '\n\n\n\n\n\n')


    def __len__(self):
        return len(self.image_paths)

    def load_image(self, index, paths):
        ...
        if image.shape[1] != self.size and image.shape[2] != int(self.size*self.width_mul):
            image = cv2.resize(image, (int(self.size*self.width_mul),  self.size))

        image = (np.transpose(image, axes=(2, 0, 1))).astype('float32')
        image = torch.FloatTensor((image - 0.5) / 0.5)
    ...
    def __getitem__(self, index):
        ...
        image = self.load_image(index, self.image_paths)

        return image#, wide, narrow_sem, wide_sem
...
class EncodingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image, path = self.load_image(index, self.image_paths)

        return image, path#, wide, narrow_sem, wide_sem
