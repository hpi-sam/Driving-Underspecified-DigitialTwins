...
class styleVAEGAN(nn.Module):
    def __init__(self, size, n_mlp=8, channel_multiplier=2, args=None, input_channel=3, output_channel=3):
        super().__init__()
        self.z_dim = 64
        self.args = args
        lr_mlp = 0.01
        blur_kernel = [1,3,3,1] # CHANGE
        self.width_mul = 3.5
        if check_arg(args, 'width_mul'):
            self.width_mul = args.width_mul

        #self.width_mul = 1 # drivegan original
        self.width_mul = 3.5 # masterproject
        ...
        self.enc.append(ConvLayer(input_channel, channels[size], 1))
        ...
        self.to_rgb1 = ToRGB(channels[self.constant_input_size], self.style_dim, upsample=False, out_channel=output_channel)
        ...
        for i in range(starting_i, log_size + 1):
            ...
            self.to_rgbs.append(ToRGB(out_channel, self.style_dim, out_channel=output_channel))
            in_channel = out_channel
    ...
    def forward(self, input, replace_input={}, decode_only=False, return_latent_only=False, train=True):
        ...
        if decode_only:
            ...
            image = skip[:, :3, :, :]
            image_segm = skip[:, 3:, :, :]
            res = {'image': image, 'image_segm': image_segm, 'spatial_z': spatial_z, 'theme_z': theme_z,\
        ...
        for enc_block in self.enc:
            ...
            if s.shape[2] == self.theme_spat_dim:
                #print('theme', s.shape)
                #torch.Size([16, 512, 32, 112])
                theme_param = self.enc_theme_sampler(s)

        # 3.5
        # block torch.Size([16, 3, 256, 896])
        # block torch.Size([16, 128, 256, 896])
        # block torch.Size([16, 256, 128, 448])
        # block torch.Size([16, 512, 64, 224])
        # theme torch.Size([16, 512, 32, 112])
        # block torch.Size([16, 512, 32, 112])
        # block torch.Size([16, 512, 16, 56])
        # block torch.Size([16, 512, 8, 28])

        # 1
        # block torch.Size([16, 3, 256, 256])
        # block torch.Size([16, 128, 256, 256])
        # block torch.Size([16, 256, 128, 128])
        # block torch.Size([16, 512, 64, 64])
        # theme torch.Size([16, 512, 32, 32])
        # block torch.Size([16, 512, 32, 32])
        # block torch.Size([16, 512, 16, 16])
        # block torch.Size([16, 512, 8, 8])

        ## get spatial z
        gen_input = self.to_vq_input(s)
        spatial_z, spatial_mu, spatial_logvar = self.reparameterize(gen_input, train=train)

        #print(theme_param.shape)
        #torch.Size([1296, 256])
        ...
        ########################## decoding phase #########################
        #import sys
        #print(spatial_z.shape)
        #print(theme_z.shape)
        #torch.Size([16, 64, 4, 14])
        #torch.Size([1296, 128])
...
class Discriminator(nn.Module):
    def __init__(self, size, channel_multiplier=2, channel_multiplier2=1, blur_kernel=[1, 3, 3, 1], args=None, input_channels=3):
        super().__init__()
