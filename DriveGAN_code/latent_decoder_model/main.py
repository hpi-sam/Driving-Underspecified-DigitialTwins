    ...
    if get_rank() == 0:
        if i % 25 == 0:
            ...
            for key, val in loss_reduced.items():
                ...
                wandb_data['train/'+key] = loss_reduced[key]
        ...
        if i % 1000 == 0:
            ...
            with torch.no_grad():
                ...
                wandb.log({"Real Image": wandb.Image(real_img)})
                wandb.log({"Reconstructed Image": wandb.Image(recon_img)})
                ...
                wandb.log({"Random Theme": wandb.Image(out['image'])})
                # random spatial_z
                ...
                wandb.log({"Random Spatial": wandb.Image(out['image'])})
                # random generation
                ...
                wandb.log({"Random Generation": wandb.Image(out['image'])})
                del out_dict
    del out
    return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iter', type=int, default=160001)
    ...
    parser.add_argument('--width_mul', type=float, default=3.5)
    ...
    if args.ckpt is not None:
        ...
        try:
            ...
            args.start_iter = 80000 #int(os.path.splitext(ckpt_name)[0].split('_')[1])
        except ValueError:
            pass

        state_dict_new = ckpt['vae']
        state_dict_old = vae.state_dict()
        ...
        vae.load_state_dict(state_dict_new, strict=False)

        state_dict_new = ckpt['d']
        state_dict_old = discriminator.state_dict()
        for key, _ in state_dict_old.items():
            if state_dict_old[key].shape != state_dict_new[key].shape:
                print('different', key, state_dict_new[key].shape, state_dict_old[key].shape)
                state_dict_new[key] = state_dict_old[key]
        discriminator.load_state_dict(state_dict_new, strict=False)


        state_dict_new = ckpt['vae_ema']
        state_dict_old = vae_ema.state_dict()
        for key, _ in state_dict_old.items():
            if state_dict_old[key].shape != state_dict_new[key].shape:
                print('different', key, state_dict_new[key].shape, state_dict_old[key].shape)
                state_dict_new[key] = state_dict_old[key]
        vae_ema.load_state_dict(state_dict_new, strict=False)

        # state_dict_new = ckpt['vae_optim']
        # state_dict_old = vae_optim.state_dict()
        # for k, d in state_dict_old.items():
        #     print(d)
        #     for key, _ in d.items():
        #         if state_dict_old[k][key].shape != state_dict_new[k][key].shape:
        #             print('different', key, state_dict_new[k][key].shape, state_dict_old[k][key].shape)
        #             state_dict_new[k][key] = state_dict_old[k][key]
        # vae_optim.load_state_dict(state_dict_new, strict=False)

        # state_dict_new = ckpt['d_optim']
        # state_dict_old = d_optim.state_dict()
        # for k, d in state_dict_old.items():
        #     for key, _ in d.items():
        #         if state_dict_old[k][key].shape != state_dict_new[k][key].shape:
        #             print('different', key, state_dict_new[k][key].shape, state_dict_old[k][key].shape)
        #             state_dict_new[k][key] = state_dict_old[k][key]
        # d_optim.load_state_dict(state_dict_new, strict=False)
    ...
    img_dataset = ImagesDataset(args.path, args.dataset, args.size, train=True, args=args)
    ...
    val_dataset = ImagesDataset(args.val_path, args.dataset, args.size, train=False, args=args)
