...
class EngineGenerator(nn.Module):
    ...
    def run_step(self, state, h, c, action, batch_size,
        ...
        #print('State Run Step', state[0][0])
    ...
    def forward(self, states, actions, warm_up, train=True, epoch=0, \
                    logvars=None, logvars_style=None, logvars_theme=None, latent_decoder=None, force_theme=None, force_style=None):
        '''
        ...
        ##### run warm_up stage
        d = self.run_warmup(states, actions, warm_up, latent_decoder=latent_decoder, force_style=force_style)
        ...
        for i in range(warm_up, len(actions) - 1):
            ...
            #     style_h = force_theme.repeat(1,14).unsqueeze(0).to('cuda')
            d = self.run_step(prev_z, h, c, actions[i], batch_size, step=i, force_input_grad=i == 0,
                        ...
                        logvars_theme=logvars_theme, latent_decoder=latent_decoder, force_style=force_style)
            prev_z, h, c, style_h = d['prev_z'], d['h'], d['c'], d['style_h']
            print('STYLE H', style_h.shape)
            outputs.append(d['prev_z'])
            z_adeps.append(d['z_adep'])
            z_aindeps.append(d['z_aindep'])
            z_themes.append(d['z_theme'])
